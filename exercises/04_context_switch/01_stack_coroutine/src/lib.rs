...
impl TaskContext {
    ...
    pub fn init(&mut self, stack_top: usize, entry: usize) {
        self.ra = entry;
    // 强制 16 字节对齐：将地址的低 4 位清零 (stack_top & !15)
        self.sp = stack_top & !0xf; 
    // s0-s11 保持 0 即可，新任务开始时它们通常不需要特定值
    }
}
...
pub unsafe fn switch_context(old: &mut TaskContext, new: &TaskContext) {
    core::arch::naked_asm!(
        // ======= 1. 保存当前 (old) 任务的状态 =======
        "sd ra, 0(a0)",       // 将返回地址存在 old.ra
        "sd sp, 8(a0)",       // 将栈指针存在 old.sp
        "sd s0, 16(a0)",      // 开始保存 s0-s11
        "sd s1, 24(a0)",
        "sd s2, 32(a0)",
        "sd s3, 40(a0)",
        "sd s4, 48(a0)",
        ...
        // ======= 2. 加载目标 (new) 任务的状态 =======
        "ld ra, 0(a1)",       // 恢复新任务的返回地址
        "ld sp, 8(a1)",       // 切换到新任务的栈
        "ld s0, 16(a1)",      // 恢复新任务的 s0-s11
        "ld s1, 24(a1)",
        "ld s2, 32(a1)",
        "ld s3, 40(a1)",
        "ld s4, 48(a1)",
        ...
        // ======= 3. 清理并返回 =======
        "li a0, 0",           // 清空 a0，防止泄露旧任务地址
        "li a1, 0",           // 清空 a1
        "ret"                 // 此时 ra 已经是新任务的地址了，ret 直接跳过去
    );
}
